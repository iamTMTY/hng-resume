    ...
    <>
      ...
      <Modal open={nav} handleClose={() => {setNav(false)}}>
        <div className="max-w-[90vw] flex flex-col justify-center items-center rounded-2xl gap-8 max-h-[90vh] w-[400px] h-[400px] bg-tertiary">
          {
            pages.map((page, i) => (
              <motion.div key={page || "Home"} initial={{x: -20, opacity: 0}} animate={{x: 0, opacity: 1}} transition={{delay: 0.1*i}}>
                <Link to={"/" + page.toLocaleLowerCase()} className={`${page.toLocaleLowerCase() === title ? "text-accent" : "hover:text-secondary"}`}>{page || "Home"}</Link>
              </motion.div>
            ))
          }
        </div>
